...
$(function () {
    ...
    $('.comment_list_con').delegate('a,input', 'click', function () {
        ...
        if (sHandler.indexOf('reply_sub') >= 0) {
            ...
            var comment = $this.prev().val()//next
            ...
            $.ajax({
                ...
                success: function (resp) {
                    if (resp.errno == "0") {
                        //将数据显示在页面
                        $this.parent().siblings(".comment_time").before($('<div class="reply_text_con fl"><div class="user_name2">用户' + resp.backdata.user + '</div><div class="reply_text">'+ comment +'</div></div>'))

                        // back_comment.css({background: "red"})
                        // 请空输入框
